...
![image](http://latex.codecogs.com/gif.latex?\\\hapDiv=\sum_{i=1}^{n}\sum_{j=i}^{n}x_{i}x_{j}d_{ij})
...
![image](http://latex.codecogs.com/gif.latex?\\\hapDist_{ab}=\frac{1}{2}\sum_{i=1}^{n}\sum_{j=1}^{n}x_{ai}x_{bj}d_{ij})

where definition of n, x and d in the equation are the same with those in the equation of hapDiv.

Then:

![image](http://latex.codecogs.com/gif.latex?\\\HFI_{ab}=hapDist_{ab}-\min(hapDiv_{a},hapDiv_{b}))
